...
- **全文搜索**：`#PID` / 关键词 / 标签
- **帖子详情**：完整正文、回复 / 关注 / 赞 / 踩、标签、**帖内图片**（本地永久归档、点击放大灯箱）、**已缓存留言**（洞主标识、楼层、点赞）
- **图片归档**：树洞原帖里的图片随抓取一并下载到本地永久保存，源站删图后本地仍可看；详情页内嵌展示
...
TREEHOLE_ARCHIVE_DAYS=365               # 归档回溯天数（决定最早抓到哪天）
TREEHOLE_ARCHIVE_MAX_PAGES=5000        # 归档分页上限
TREEHOLE_ARCHIVE_SLICE_PAGES=1        # 每次归档抓多少页（调大可加快回溯）
TREEHOLE_ARCHIVE_INTERVAL_MS=15000
TREEHOLE_COMMENT_LIMIT_ARCHIVE=100
TREEHOLE_ARCHIVE_FORCE_RESUME=0       # 设 1 可让已“完成”的归档带着更大的天数/页数上限续抓更早历史
...
# 帖内图片归档
TREEHOLE_IMAGE_ARCHIVE_ENABLED=1                                          # 总开关
TREEHOLE_IMAGE_BASE=https://treehole.pku.edu.cn/chapi/api/v3/media/getMediaBinary  # 图片二进制端点（一般无需改）
TREEHOLE_IMAGE_CONCURRENCY=3            # 下载并发
TREEHOLE_IMAGE_DELAY_MS=250            # 每张图之间的礼貌间隔
TREEHOLE_IMAGE_MAX_BYTES=26214400     # 单图上限（默认 25MB）
TREEHOLE_IMAGE_MIN_FREE_GB=5          # 磁盘可用空间低于此值时暂停下载
...
| `data/media/<YYYY-MM>.json` | 按月保存帖子图片清单（pid → 本地图片文件 / 字节 / 来源） |
| `data/media/files/<YYYY-MM>/<pid>-<序号>.<ext>` | 下载保存的图片原文件，经 `/media/...` 公开访问 |
| `data/media/status.json` | 图片归档累计进度 |
| `data/hot-cache.json` | 旧版本遗留缓存，仅用于迁移 |

存储层用 `stream-json` 流式解析大分片，避免一次性载入整月数据；并对已载入分片做有界 LRU 管理。图片元数据走独立的按月 sidecar（`data/media/`），不写入帖子分片热路径。

> 图片归档可观：图片帖约占缓存帖的 9%，按均值 0.5–1.5 MB/张计，全量约几十 GB；`TREEHOLE_IMAGE_MIN_FREE_GB` 会在磁盘吃紧时自动暂停下载。

## 后台任务与脚本

服务运行时自动执行：高频增量同步、按 `TREEHOLE_BACKFILL_INTERVAL_MS` 的深页补抓、（启用时）后台一年归档、每日被删帖比对、帖内图片的后台下载归档。
...
npm run media-scan     # 命令行回填本地图片帖的图片（best-effort，需登录态）
```
...
- `GET /api/post/:pid` — 帖子详情、已归档帖内图片（`images`）与已缓存留言
- `GET /media/<YYYY-MM>/<pid>-<序号>.<ext>` — 已归档图片原文件（静态、长缓存、公开）
...
src/
  ...
  mediaArchiver.js   帖内图片后台下载与按月 sidecar 归档（data/media/）
  cli-crawl.js / smoke.js / migrate-cache.js / warm-summaries.js / deleted-scan.js / media-scan.js  运维脚本
public/
